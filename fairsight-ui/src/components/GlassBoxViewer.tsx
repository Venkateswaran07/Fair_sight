...
const DecisionBadge = ({ decision }: { decision: 'APPROVED' | 'REJECTED' }) => {
...
const SHAPBarRow = ({ feat, maxAbsShap, proxySet }: { feat: SHAPFeature; maxAbsShap: number; proxySet: Set<string> }) => {
...
const SHAPSection = ({ top_features, proxy_flags }: { top_features: SHAPFeature[]; proxy_flags: string[] }) => {
...
const CounterfactualsSection = ({ counterfactuals, decision }: { counterfactuals: string[]; decision: 'APPROVED'|'REJECTED' }) => {
...
const ExplanationSection = ({ plain_explanation }: { plain_explanation: string }) => {
  if (!plain_explanation) return null
  return (
    <div>
      <SectionHeading title="Plain English Explanation" subtitle="Generated by Gemini 1.5 Flash via Vertex AI — summarises the decision for a non-technical reader." />
      <blockquote className="relative px-6 py-5 bg-white border-l-4 border-blue-700 border border-t border-r border-b-gray-200 rounded-r-lg">
        ...
        <footer className="mt-3 pl-4">
          <span className="text-xs text-gray-400 font-medium tracking-wide uppercase">Google GenAI · Gemini 2.0 Flash</span>
        </footer>
      </blockquote>
    </div>
  )
...
const ReviewActions = ({ decision, applicantId, onReviewed }: { decision: 'APPROVED'|'REJECTED'; applicantId?: string | null; onReviewed?: () => void }) => {
  ...
  async function handleAction(status: "flagged" | "reviewed") {
    ...
    try {
      ...
      setState({ loading: false, done: null, error: message })
    }
  }
