 ...
 * GlassBoxViewer
 * ==============
 * The centrepiece screen: shows a single applicant's full explainability report.
 *
 * Props
 * -----
 * decision          "APPROVED" | "REJECTED"
 * top_features      [{feature, shap_value, contribution_percent, direction?}]
 * proxy_flags       string[]   feature names flagged as proxies
 * counterfactuals   string[]   plain "If X were Y: APPROVED" strings
 * plain_explanation string     Groq LLM narrative
 * applicantId       string?    forwarded to the review endpoint (optional)
 * onReviewed        () => void optional callback after a review action
 */
  ...
  return (
    <div className={[
      ...
      <span className={[
        ...
        approved ? 'text-green-800' : 'text-red-800',
      ].join(' ')}>
        {decision}
      </span>
...
/* ════════════════════════════════════════════════════════════════════════
   ...
   Each row has a center axis; bars extend LEFT (negative) or RIGHT (positive).
════════════════════════════════════════════════════════════════════════ */
  ...
  return (
    <div className="flex items-center justify-center gap-6 mb-4 text-xs text-gray-500">
      <div className="flex items-center gap-1.5">
        ...
        <span>Pushed toward rejection</span>
      </div>
...
function SHAPBarRow({ feat, maxAbsShap, proxySet }) {
  const isProxy    = proxySet.has(feat.feature)
  const isPositive = feat.shap_value >= 0
  const halfPct    = maxAbsShap > 0
    ? (Math.abs(feat.shap_value) / maxAbsShap) * 100
    : 0
  ...
  return (
    <div className="flex items-center gap-2 py-2.5 border-b border-gray-100 last:border-0">
      ...
      {/* Feature label — right-aligned into the chart's left margin */}
      <div className="w-[28%] flex items-center justify-end gap-1.5 pr-2 flex-shrink-0 min-w-0">
        {isProxy && (
          <span
            title="This feature may be a proxy for a protected attribute"
            className="text-amber-500 flex-shrink-0 text-base leading-none"
            aria-label="Proxy risk warning"
          >
            ⚠
          </span>
        )}
        <span className={[
          ...
          isProxy ? 'text-amber-700 font-semibold' : 'text-gray-800',
        ].join(' ')}>
          {feat.feature}
        </span>
      </div>

      {/* Diverging chart — left half + center + right half */}
      <div className="flex-1 flex items-center min-w-0">
        ...
        <div className="flex-1 flex justify-end items-center h-7 min-w-0">
          {!isPositive && (
            <div
              className="h-5 bg-red-500 rounded-l-sm transition-all duration-300"
              style={{ width: `${halfPct}%` }}
              aria-label={`Negative SHAP ${shapFormatted}`}
            />
          )}
        </div>
        ...
        <div className="flex-1 flex justify-start items-center h-7 min-w-0">
          {isPositive && (
            <div
              className="h-5 bg-green-500 rounded-r-sm transition-all duration-300"
              style={{ width: `${halfPct}%` }}
              aria-label={`Positive SHAP ${shapFormatted}`}
            />
          )}
        </div>
      </div>
      ...
      {/* SHAP value */}
      <div className={[
        'w-14 text-right text-xs font-mono flex-shrink-0',
...
function SHAPSection({ top_features, proxy_flags }) {
  const [search, setSearch] = useState('')
  
  if (!top_features?.length) return null

  const proxySet   = new Set(proxy_flags ?? [])
  const filtered   = top_features.filter(f => f.feature.toLowerCase().includes(search.toLowerCase()))
    ...
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 mb-6">
        <SectionHeading
          ...
          subtitle="How each feature moved the model's decision — measured by SHAP value."
        />
        <div className="relative w-full sm:w-48">
          <input 
            ...
            className="w-full pl-8 pr-3 py-1.5 border border-gray-300 rounded-md text-xs outline-none focus:ring-1 focus:ring-accent bg-white"
          />
      ...
      {proxy_flags?.length > 0 && (
        <div className="mt-3 flex gap-2 px-4 py-3 bg-amber-50 border border-amber-200 rounded-lg text-sm text-amber-800">
          ...
          <span>
            ...
            {proxy_flags.join(', ')}{' '}
            {proxy_flags.length === 1 ? 'is' : 'are'} statistically correlated with
            protected attributes and may encode indirect bias.
          </span>
        </div>
      )}
    </div>
...
function CounterfactualsSection({ counterfactuals, decision }) {
        ...
        {counterfactuals.map((cf, i) => (
          ...
          >
            ...
            <span className="flex-shrink-0 flex items-center justify-center
                             w-5 h-5 rounded-full bg-blue-100 text-blue-700
...
function ExplanationSection({ plain_explanation }) {
  ...
  return (
    <div>
      <SectionHeading
        ...
        subtitle="Generated by Gemini 2.0 Flash via Google GenAI — summarises the decision for a non-technical reader."
      />

      <blockquote className="relative px-6 py-5 bg-white border-l-4 border-blue-700
                             border border-gray-200 rounded-r-lg">
        {/* Open quote */}
        <span className="absolute top-3 left-3 text-4xl text-blue-100 font-serif
                         leading-none pointer-events-none select-none" aria-hidden>
          "
        </span>

        <p className="text-gray-800 text-[0.95rem] leading-relaxed pl-4">
          {plain_explanation}
        </p>
...
function ReviewActions({ decision, applicantId, onReviewed }) {
  const [state, setState] = useState({
    loading: false,
    done: null,       // "flagged" | "reviewed"
    error: null,
  })
  ...
  if (state.done) {
    return (
      <div className={[
        ...
        {state.done === 'flagged'
          ? 'Flagged as concerning — a reviewer will follow up.'
          : 'Marked as reviewed — no further action needed.'}
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <button
          ...
          className="flex-1 flex items-center justify-center gap-2
                     ...
                     font-medium text-sm px-5 py-2.5 rounded-lg transition-colors"
        >
          {state.loading ? (
            <Spinner size={14} label="Submitting…" />
          ) : (
            /* Flag icon */
      ...
      <p className="text-xs text-gray-400 text-center">
        Review status is posted to{' '}
        <code className="bg-gray-100 px-1 rounded">POST /audit/review</code>.
      </p>
    </div>
  )
}
 ...
 *   plain_explanation="The applicant was rejected primarily because…"
 *   applicantId="app_001"
 * />
 * ```
 */
export default function GlassBoxViewer({
  ...
  return (
    <article
      className="max-w-2xl mx-auto bg-[#F9F9F7] font-sans"
      aria-label="Applicant fairness explanation"
    >
      {/* ── 1. Decision Badge ─────────────────────────────────── */}
      ...
      <div className="mt-8 flex flex-col gap-8">
        ...
        <div>
          <SectionHeading
            ...
            subtitle="Record your assessment of this decision."
          />
          <ReviewActions
            decision={decision}
